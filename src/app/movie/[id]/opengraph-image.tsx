...
export default async function Image({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!movie) {
    // Falls through to nothing special - Next just uses the parent
    // app/opengraph-image.tsx (the site-wide branded card) if this
    // route throws/returns nothing usable for a bad id.
    return new ImageResponse(<div style={{ width: '100%', height: '100%', background: '#121316' }} />, size);
  }

  const backdrop = movie.backdrop_path ? tmdbImage(movie.backdrop_path, 'w780') : null;
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 20, fontSize: 24 }}>
            ...
            {movie.genres?.[0] && (
              <span style={{ color: 'rgba(237,237,239,0.55)' }}>{movie.genres[0]}</span>
            )}
          </div>
        </div>
      </div>
