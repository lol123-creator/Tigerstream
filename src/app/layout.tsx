    ...
    <html lang="en">
      <body className={`min-h-screen font-sans ${inter.variable}`}>
        ...
        <footer className="border-t border-white/5 py-10 text-sm text-white/35">
          <div className="mx-auto max-w-7xl px-4 sm:px-6"><div className="flex flex-wrap justify-center gap-x-6 gap-y-2"><a href="/" className="hover:text-white/60 transition">Home</a><a href="/movies" className="hover:text-white/60 transition">Movies</a><a href="/tv" className="hover:text-white/60 transition">TV Shows</a><a href="/anime" className="hover:text-white/60 transition">Anime</a><a href="/sports" className="hover:text-white/60 transition">Sports</a><a href="/genres" className="hover:text-white/60 transition">Genres</a><a href="/favorites" className="hover:text-white/60 transition">My List</a></div><p className="mt-4 text-center">{SITE_NAME}</p></div><div className="mt-6 border-t border-white/5 pt-6 text-center text-xs leading-relaxed text-white/25 space-y-1"><p className="font-semibold text-white/40 text-sm">Important Disclaimer</p><p className="text-base">◝(ᵔᵕᵔ)◜</p><p>TigerStream operates as a content aggregator and does not host any media files on our servers. All content is sourced from third-party providers and embedded services. For any copyright concerns or DMCA takedown requests, please contact the respective content providers directly.</p></div></div>
        </footer>
        <Analytics />
        <SpeedInsights />
      </body>
